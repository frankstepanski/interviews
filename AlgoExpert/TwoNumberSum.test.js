...
const {TwoNumberSum, TwoNumberSum1 } = require('./TwoNumberSum.js')
// non-empty array of distinct integers and an integer representing a target sum.
test("TwoNumberSumTest brute solution with O(n^2)", () =>{
    expect(TwoNumberSum(array, targetSum)).toEqual([4,6])
})
test("TwoNumberSumTest solution 2 with O(n)", () =>{
    expect(TwoNumberSum1(array, targetSum)).toEqual([4,6])
})
